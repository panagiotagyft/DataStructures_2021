...
link NEW(Item item,char *s1, char *s2, link l, link r, int N)
  { link x = malloc(sizeof *x);
    x->item = item; 
    	x->FirstName=(char*)malloc(strlen(s1)+1);  // allocate memory for x->FirstName
    	x->LastName=(char*)malloc(strlen(s2)+1);   // allocate memory for x->LastName
    	strcpy(x->FirstName,s1);
        strcpy(x->LastName,s2);
...
// insert the new node into the root of a tree. The new element inserted ino the root of the appropriate subtree
link insertT(link h, Item item, char* s1, char* s2)   
  { Key v = key(item);       
    if (h == z) return NEW(item, s1, s2, z, z, 1);
    if (less(v, key(h->item)))
      { h->l = insertT(h->l, item, s1, s2); h = rotR(h); }
    else
      { h->r = insertT(h->r, item, s1, s2); h = rotL(h); }
    return h;
  }
int Fix_N(link h){
	int i, j;
...
void STinsert(Item item, char* s1, char* s2){
    head = insertT(head, item, s1, s2); 
     Fix_N(head);
  }

link insertR(link h, Item item, char* s1, char* s2)
  { Key v = key(item), t = key(h->item);
    if (h == z) return NEW(item, s1, s2, z, z, 1); // if tree is null --> return a new node that contains the element
    if less(v, t)                       //if the key is smaller than the root key --> i give to the left branche the item  
         h->l = insertR(h->l, item, s1, s2);
    else h->r = insertR(h->r, item, s1, s2);  // else  i give to the right branche the item  
    (h->N)++; return h;       //increase the N
  }

void STinsert1(Item item,char* s1, char* s2)
  { head = insertR(head, item, s1, s2); }
...
link joinLR(link a, link b)  //the smaller node of the right subtree becomes root and then the left branche point to the lrft subtree
  {
    if (b == z) return a;
    b = partR(b, 0); b->l = a;
    return b;
...
void STdeleteAll(Key v)
  { head = deleteR(head, v); 
    Fix_N(head);	  //i fix N
    if (STsearch(v) != NULLitem){   //if there is more same element -->> delete 
    	STdeleteAll(v);
	}
  }
...
void print(link h, int count){
    ...
    printf("{%3d %4s %4s %3d }\n",h->item, h->FirstName, h->LastName, h->N); // i print the current node after space
