...
NodeType *union_2lists(NodeType *A, NodeType *B){
	NodeType *C, *N, *K;
	
	A=ListSort(A);   // I sort the list A
	B=ListSort(B);   // I sort the list B
	N=A;         // My first pointer (previous)
	K=A; 
	K=K->next;   // My second pointer (current)
	...
	while(K!=NULL){   // I cross the list until i reach the end
		N=N->next;     
		K=K->next;
	}
	N->next=B;         // The next pointer of the node N point to list B
	C=A;               // I initialize the C 
	
	C=ListSort(C);   // I sort the list C
	
    return C;        // I return the list C
